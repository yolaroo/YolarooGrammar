...
#import "WordTokenMatchView.h"

#import "MainFoundation+WordTokenDataLoader.h"

@interface WordTokenMatchView ()
...
@property (strong,nonatomic) NSMutableString* theHiddenString;
...
@implementation WordTokenMatchView
...
#pragma mark - Buttom Button Press
...
- (void) viewChangeToCorrectSentence
{
    [UIView transitionWithView:self.labelTop
                    ...
                    animations:^{
                        self.labelTop.text = self.theHiddenString;
...
- (void) setTheSemanticDictionaryData {
    self.theDictionaryData = [self myDataForTokenType:self.managedObjectContext];
}
...
- (void) loadDataToUI {
    ...
    @try {
        ...
        NSArray* answer;
        answer = [self simpleSentenceLoadForTokenType:self.theDictionaryData withContext: self.managedObjectContext];
        self.theAnswer = [NSMutableString stringWithString:[[answer lastObject] firstObject]];
        
        while ([self.theLastAnswerArray containsObject:[answer objectAtIndex:1]]){
            answer = [self simpleSentenceLoadForTokenType:self.theDictionaryData withContext: self.managedObjectContext];
        ...
        [self.theLastAnswerArray addObject:[answer objectAtIndex:1]];
        self.theHiddenString = [answer objectAtIndex:1];
