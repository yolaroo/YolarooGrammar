//
//  AdjectiveListFromSemanticTypeView.m
//  YolarooGrammar
//
...
@interface AdjectiveListFromSemanticTypeView ()
...
@implementation AdjectiveListFromSemanticTypeView
...
- (UITableViewCell *) setMyCell: (UITableViewCell*) cell cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    AdjectiveClass * theAdjective = [self.fetchedRecordsArray objectAtIndex:indexPath.row];
    if (theAdjective != nil){
        cell.textLabel.text = [NSString stringWithFormat:@"%@ - (%@)",[self sentenceSpaceFixer:theAdjective.basic withCaps:YES], theAdjective.whatSemanticType.name];
        
        cell.detailTextLabel.text = [NSString stringWithFormat:@"%@ - %@",theAdjective.comparative, theAdjective.superlative];
...
- (NSArray*) myArraySetter {
    @try {
        NSArray*mine = [[self selectedAdjectiveListFromSemanticType :self.selectedRecord withContext: self.managedObjectContext]copy];
