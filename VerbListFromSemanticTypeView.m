//
//  VerbListFromSemanticTypeView.m
//  YolarooGrammar
//
...
@interface VerbListFromSemanticTypeView ()
...
@implementation VerbListFromSemanticTypeView
...
- (UITableViewCell *) setMyCell: (UITableViewCell*) cell cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    VerbWord * theVerb = [self.fetchedRecordsArray objectAtIndex:indexPath.row];
    if (theVerb != nil){
        cell.textLabel.text = [NSString stringWithFormat:@"%@ - %@ - %@",theVerb.infinitive, theVerb.past, theVerb.perfect];
        
        cell.detailTextLabel.text = [NSString stringWithFormat:@"%@ - %@ - %@",theVerb.simple, theVerb.thirdPersonSingular, theVerb.gerund];
        return cell;
    }
    else {
        return nil;
...
- (NSArray*) myArraySetter {
    @try {
        NSArray*mine = [[self selectedVerbListFromSemanticType :self.selectedRecord withContext: self.managedObjectContext]copy];
