...
#import "PossessiveSentenceView.h"
#import "MainFoundation+PossessiveDataLoader.h"

@interface PossessiveSentenceView ()
...
@implementation PossessiveSentenceView
...
- (NSDictionary*) dataLoad {
    // delta
    return [self theDataSetFromCaseForPossessive:self.theCase withContext:self.managedObjectContext];
}

- (NSArray*) buttonDataLoad {
    // delta
    NSArray*myFirstButtons = [self buttonViewFromCaseForPossessive:self.theCase];
...
- (void) loadDataToUI {
    @try {
        [self resetUI];
        // delta
        NSArray* answer = [[self dataTransformationForPossessive:[self dataLoad]]copy];
