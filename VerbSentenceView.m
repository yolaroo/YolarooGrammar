...
#import "VerbSentenceView.h"

#import "MainFoundation+VerbDataLoaders.h"

@interface VerbSentenceView ()
...
@implementation VerbSentenceView
...
- (IBAction)topButtonPress:(UIButton *)sender {
    if (self.canClickTopButton) {
        ...
        NSString* theNewSentence = [theSentence stringByReplacingOccurrencesOfString:@"_____" withString:@""];
        [self foundationRunSpeech:@[theNewSentence]];
    }
}
...
- (void) checkButtonPress:(UIButton*)myButton {
    ...
    if ([self.theAnswer isEqualToString:string]){
        ...
        self.labelTop.text = self.theReformatedAnswer;
        [self foundationRunSpeech:@[self.theReformatedAnswer]];
...
- (NSDictionary*) dataLoad {
    //delta
    return [self theDataSetFromCaseForVerb:self.theCase withContext:self.managedObjectContext];
}

- (NSArray*) buttonDataLoad {
    //delta
    NSArray*myFirstButtons = [self buttonViewFromCaseForVerbs:self.theCase];
...
- (void) buttonViewSet {
    NSArray*myButtons = @[self.button01,self.button02,self.button03,self.button04,self.button05];
...
- (void) loadDataToUI {
    @try {
        [self resetUI];
        NSArray* answer = [[self dataTransformationForVerb:[self dataLoad]]copy];
        [self loadView: answer];
        self.theAnswer = [NSMutableString stringWithString:[answer lastObject]];
        self.theReformatedAnswer = [NSMutableString stringWithString:[answer objectAtIndex:1]];
    }
